...
class GPT3(Base):
    ...
    def translate(self, text, prompt_template):
        prompt_template = prompt_template or "Please help me to translate，`{text}` to {language}"
        print(text)
        self.rotate_key()
        self.data["prompt"] = prompt_template.format(text=text, language=self.language)
